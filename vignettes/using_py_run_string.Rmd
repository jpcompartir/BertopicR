---
title: "using_py_run_string"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{using_py_run_string}
...
Check that our environment has been configured correctly
...
Now we can look at what's been loaded in. The "py$" symbol tells us what is loaded into our python environment. We should see a vector containing: backend; BERTopic; cluster; plotting; representation; vectorizers
```{r check modules}
names(py$bertopic)
```
...
Next we use the BERTopic() function to initiate the model and the fit_transform() function to fit our data. As before, we use the "\$" symbol to access functions stored in the python environment. Here we use py\$bertopic$BERTopic to access the BERTopic function within the bertopic library within the py environment. Having created the model object, we can use the "\$" symbol to call the fit_transform() function.

NOTE: fit_transform() should only be used when first training and fitting the model. Once a model has been trained, transform() cam be used to predict topics for new data.

*add something about setting random state for umap*
...
# model = BERTopic()
# 
# # Move the BERTopic model to the GPU
# # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# # model.transformer_model.to(device)
# 
# model_py = bertopic.BERTopic()
# topics, probs = model_py.fit_transform(r.data["text_clean"])
# model_py.get_topic_info()
```

```{r embedings and model}
# import sentence transformer and umap library from python
sentence_transformers <- reticulate::py_run_string("from sentence_transformers import SentenceTransformer")
# umap <- reticulate::py_run_string("from umap import UMAP")

import("sentence_transformers", convert = TRUE)
import("bertopic")
# choose sentence transformer to use and specify "mps" accelerator
sentence_model_gpu2 <- py_run_string("sentence_transformers.SentenceTransformer(\"all-MiniLM-L6-v2\", device = \"mps\")")

test <- sentence_transformers$SentenceTransformer("all-MiniLM-L6-v2")
# embed text using sentence transformer
embeddings <- py_run_string("r.sentence_model_gpu.encode(r.data.text_clean)")

# define umap with random_state set
...
# initiate the model
model <- py_run_string("bertopic.BERTopic()")

# fit the model
model_output <- py_run_string("r.model.fit_transform(documents = r.data.text_clean,
...
# device <- huggingfaceR::hf_set_device()

# model_id <- "all-MiniLM-L6-v2"
# 
# minilm <- huggingfaceR::hf_load_sentence_model(model_id)
# minilm_gpu <- minilm$to(device = device)

# bing_data <- readr::read_rds("~/Google Drive/My Drive/625 Peaks and Pits/Data/3Apr23/bing_original_data.Rds")
# 
# data200 <- bing_data %>% slice_sample(n = 200000) %>%
#   mutate(text_clean = message,
#         message = tolower(message))
# data100 <- bing_data %>% slice_sample(n = 100000)%>%
#   mutate(text_clean = message,
#         message = tolower(message))
# data50 <- data %>% slice_sample(n = 50000) %>%
#   mutate(text_clean = message,
#         message = tolower(message))
...
# torch$has_mps
# device_mps <- torch$device(device = "mps")
# 
# library(profvis)
# 
# profvis({
#   # embeddings <- sentence_model$encode(data5$text_clean)
#   embeddings_gpu5 <- sentence_model_gpu$encode(data5$text_clean)
#   embeddings_gpu50 <- sentence_model_gpu$encode(data50$text_clean)
#   model_gpu5 <- py$bertopic$BERTopic()
#   model_gpu50 <- py$bertopic$BERTopic()
#   fitted_data_gpu5 <- model_gpu5$fit_transform(data5$message, embeddings = embeddings_gpu5)
#   fitted_data_gpu50 <- model_gpu50$fit_transform(data50$message, embeddings = embeddings_gpu50)
# })
# 
# profvis({
#   embeddings_gpu100 <- sentence_model_gpu$encode(data100$text_clean)
#   embeddings_gpu200 <- sentence_model_gpu$encode(data200$text_clean)
#   model_gpu100 <- py$bertopic$BERTopic()
#   model_gpu200 <- py$bertopic$BERTopic()
#   fitted_data_gpu100 <- model_gpu100$fit_transform(data100$text_clean, embeddings = embeddings_gpu100)
#   fitted_data_gpu200 <- model_gpu200$fit_transform(data200$text_clean, embeddings = embeddings_gpu200)
# })
# 
# 
# profvis({
#   embeddings_st <- minilm$encode(data5$text_clean)
#   embeddings_gpu_st <- minilm_gpu$encode(data5$text_clean)
#   model_st <- py$bertopic$BERTopic()
#   model_gpu_st <- py$bertopic$BERTopic()
#   fitted_data_st <- model_st$fit_transform(data5$message, embeddings = embeddings)
#   fitted_data_gpu_st <- model_gpu_st$fit_transform(data5$message, embeddings = embeddings_gpu)
# })
...
```{r get docs}
# model$get_document_info(data$text_clean)
```
