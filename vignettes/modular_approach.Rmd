...
topic_model$get_topic_info() %>% select(-Representative_Docs, -Representation) %>% head(10)
```
...
```{r}
topic_model$set_topic_labels(hf_representations)
...
4. **Probabilites:** This strategy uses hdbscan soft-clustering to find the best matching topic for each outlier document. To use this you will need to get the topic-document probability matrix using bt_probability_matrix, this matrix tells you the probability of any document being in any cluster.
...
# prob_matrix <- bt_probability_matrix(fitted_model = topic_model,
#                                      hdbscan_model = clusterer)
# 
# outliers_probs <- bt_outliers_probs(fitted_model = topic_model,
#                                     documents = sentences,
#                                     topics = topic_model$topics_,
...
data %>%
  mutate(outliers_ts_sim = outliers_ts_sim$new_topics,
         ...
         outliers_probs = outliers_probs$new_topics) %>%
  filter(merged_topics == -1,
         outliers_ctfidf < 12,
         outliers_embed < 12,
         outliers_ts_sim < 12,
         outliers_probs < 12) %>%
  select(outliers_ts_sim, outliers_embed, outliers_ctfidf, outliers_probs) %>%
  ...
  theme_minimal() + 
  labs(x = "Topic", 
       y = "Number of redistributed outliers", 
...
Once you have settled on a new list of topics that you are happy with, you can update your model with those new topics. For example, if after looking at you redistributed outliers, you decided that the c-TF-IDF strategy was the most appropriate you can update your model accordingly:

```{r}
bt_update_topics(fitted_model = topic_model,
                 documents = sentences,
                 new_topics = outliers_ctfidf$new_topics)
```

And that's it! You've completed a basic topic modelling pipeline using bertopic! If you would like to have a deeper look at what else we can do using bertopic, refer to the BertopicR function documentation and the BERTopic python library, https://maartengr.github.io/BERTopic/index.html.
