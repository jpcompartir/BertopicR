...
```{r embedings and model, eval = FALSE}
output <- fit_transform_model(cleaned_text = data$text_clean)

model <- output[[1]]
embeddings1 <- output[[2]]
```

```{r embedings and model, echo = FALSE}
output <- fit_transform_model(cleaned_text = data$text_clean,
                              random_state = 42)
...
As we have already run this fit_transform_model() on the same data, we do not need to resave the embeddings, they will be the same.
...
This time we get 5 topics output which is much more manageable for analysis. Now we might need to take a closer look at them to understand better what they are about. To do this we use the makedf() function to merge the bert results with the Sprinklr export. We can then look at topics using ParseR to create bigrams, the bertopic function vizualize_term_rank() to create a top terms plot or look at exemplars by filtering the df to the topic we're interested in.
